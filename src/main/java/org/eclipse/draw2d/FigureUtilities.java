...
//	private static final Color ghostFillColor = new Color(null, 31, 31, 31);
	...
	/**
	 * Get instance for current UI session
	 * 
	 * @return
	 */
	private static FigureUtilities getInstance()
	{
		FigureUtilities instance = (FigureUtilities)RWT.getUISession().getAttribute("FigureUtilities.instance");
		if (instance == null)
		{
			instance = new FigureUtilities();
			RWT.getUISession().setAttribute("FigureUtilities.instance", instance);
		}
		return instance;
	}
	...
	{
		FigureUtilities instance = getInstance();
		instance.setFont(f);
		if (instance.metrics == null)
			instance.metrics = instance.getGC().getFontMetrics();
	 ...
	 */
	protected org.eclipse.swt.graphics.Point getTextDimension(String s,
			Font f) {
		setFont(f);
	...
	public static Dimension getTextExtents(String text, Font f) {
		return new Dimension(getInstance().getTextDimension(text, f));
	}
	...
	public static Dimension getStringExtents(String s, Font f) {
		return new Dimension(getInstance().getStringDimension(s, f));
	}
	...
	public static void getTextExtents(String s, Font f, Dimension result) {
		org.eclipse.swt.graphics.Point pt = getInstance().getTextDimension(s, f);
		result.width = pt.x;
		result.height = pt.y;
	}
	...
	public static int getTextWidth(String s, Font f) {
		return getInstance().getTextDimension(s, f).x;
	}
	...
	protected void setFont(Font f) {
		if ((appliedFont == f || ((f != null) && f.equals(appliedFont))) && !appliedFont.isDisposed())
			return;
		getGC().setFont(f);
		appliedFont = f;
		metrics = null;
